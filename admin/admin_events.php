            ...
            <div class="table-container">
                <table>
                    ...
                    if (isset($_POST['search']) && !empty(trim($_POST['search-field']))) {
                        ...
                        $event_sql = "SELECT E.*, C.* FROM events AS E JOIN clubs AS C ON E.Club_ID = C.Club_ID WHERE E.Event_name LIKE '%$search%' ORDER BY CASE WHEN E.Approval_status = 'Pending' THEN 0 ELSE 1 END, E.Approval_status ASC, Date_posted DESC";
                    } else {
                        $event_sql = "SELECT E.*, C.* FROM events AS E JOIN clubs AS C ON E.Club_ID = C.Club_ID ORDER BY CASE WHEN E.Approval_status = 'Pending' THEN 0 ELSE 1 END, E.Approval_status ASC, Date_posted DESC";
                    }
